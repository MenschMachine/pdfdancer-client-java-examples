...
## Implementation in pdfdancer-client-java-examples

- Added `src/main/java/com/tfc/pdf/pdfdancer/examples/paths/ClearPathGroupClipping.java`, a focused paths example that opens `src/main/resources/invisible-content-clipping-test.pdf`, confirms the clipped vector path `PATH_0_000004` exists on page 1, groups that path, calls `PathGroupReference.clearClipping()`, and saves the result to `output/working-with-paths/cleared_group_clipping.pdf`.
- Added `src/main/resources/invisible-content-clipping-test.pdf` as the fixture for this example so the before/after effect is visible on hidden vector content that was excluded by a clipping path.
- Updated `build.gradle.kts` to make the example runnable in this repo: added the `https://jitpack.io` repository, switched the SDK dependency to `com.pdfdancer.client:pdfdancer-client-java:DEV`, registered the `runClearPathGroupClipping` task, and included it in the `runPathsExamples` aggregate task.
- Updated `EXAMPLES.md` so the capability is documented under "Working with Paths" and exposed as an individual command via `./gradlew runClearPathGroupClipping`.
- Repo-specific scope: this examples repo demonstrates clipping removal through the grouped-path workflow only. It does not add separate clear-clipping examples for images or text in this change.
